    ...
    *)
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
          let rec reverseIt xs acc =
            match xs with
                |[] -> acc
                |a::t -> reverseIt (t) (a::acc)
          reverseIt xs []
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let xs = List.rev xs
            let rec addOne xs acc =
                match xs with
                    |[] -> acc
                    |a::t -> addOne (t) (a+1::acc)
            addOne xs []
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let xs = List.rev xs
            let rec doubleIt xs acc =
                match xs with
                    |[] -> acc
                    |a::t -> doubleIt (t) (a*2::acc)
            doubleIt xs []
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let xs = List.rev xs
            let rec applyIt xs acc =
                match xs with
                    |[] -> acc
                    |a::t -> applyIt (t) (f a::acc)
            applyIt xs [] 
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let xs = List.rev xs
            let rec filterIt xs acc =
                match xs with
                    |[] -> acc
                    |h::t -> match f h with
                                |true -> filterIt (t) (h::acc)
                                |_ -> filterIt (t) (acc)
            filterIt xs []
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let xs = List.rev xs
            let rec filterOdd xs acc =
                match xs with
                    |[] -> acc
                    |h::t -> match (h%2=0) with
                                |true -> filterOdd (t) (acc)
                                |_ -> filterOdd (t) (h::acc)
            filterOdd xs []
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec foldIt xs acc =
                match xs with
                    |[] -> acc
                    |element::t -> foldIt t (acc+element)
            foldIt xs initialState
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec foldIt xs acc =
                match xs with
                    |[] -> acc
                    |element::t -> foldIt t (f acc element)
            foldIt xs initialState
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let xs = List.rev xs
            let rec partitionIt xs (acc1,acc2) =
                match xs with
                    |[] -> (acc1,acc2)
                    |h::t -> match f h with
                                | true -> partitionIt t (h::acc1,acc2)
                                | _ -> partitionIt t (acc1,h::acc2)
            partitionIt xs ([],[])
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec initDoIt n acc =
                match n<0 with
                    |true -> acc
                    |_ -> initDoIt (n-1) (f n::acc)
            initDoIt (n-1) []
    ...
    *)
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let xs = List.rev xs
            let rec doIt xs idx acc =
                match xs with
                    |[] -> acc
                    |h::t -> doIt t (idx-1) (f idx h::acc)
            doIt xs ((List.length xs)-1) []
