...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | _ -> "A thinker, eh?"
        randomOpinion (BSc (Statistics, ComputerScience)) |> should equal "Good choice!"
        randomOpinion (BSc (Statistics,Mathematics))  |> should equal "!!SCIENCE!!"
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a =  // <-- you may want to spread this over multiple lines and/or let-bindings ...!
            let node1=Empty
            let node2 = Node ("me",node1,node1)
            let node3 = Node ("nje",node1,node2)
            let node4 = Node ("me",node3,node3)
            let node5 = Node ("nje",node3,node4)
            node5
